...
public class InputManager : MonoBehaviour {
	...
	void Start () {
		...
		//ups
		rightUp = new Vector3(1, 1, 0); // 110
		leftUp = new Vector3(0, 0, 1); // 011

		//downs
		rightDown = new Vector3(0, 0, -1); // 0 -1 0
		leftDown = new Vector3(-1, 0, 0); //-1-10
	...
	void Update () {
	 ...
	 /*private bool isFree(KeyCode playerPressed){
		...
		foreach (var gameObj in FindObjectsOfType(typeof(GameObject)) as GameObject[]){
			if(gameObj.name.Contains("BaseTile")){
